...
\begin{definition}[graded ring]
  A \define{graded ring} is a ring $R=⊕ _{g∈ G}R_g$ such that $R_{g} × R_{g'}=R_{gg'}$.
\end{definition}

\begin{example}
  \begin{itemize}
    \item the \define{trivial gradation} $R_0=R,R_i=0$ for $i≠0$.
...
\begin{definition}[monoidal category]
  ...
  Let $a$.
  A category $C$ equipped with a functor called the \define{tensor product} in the product category of $C$ with itself
\end{definition}
